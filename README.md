...
* Suppose an organisation does not want to rely on ADCS. In that case, it might also be worthwhile to offer HTTP bulk provisioning. An administrator of eduVPN can download multiple configuration files via the eduVPN API. Next we can deploy those configuration files via the Intune API.

...
The arguments `-p` you must specify the VPN profile and `-s` you must specify the hostname of the VPN server.
...
* Suppose an organisation does not want to rely on ADCS or an MDM provider. In that case, it might also be worthwhile to offer Kerberos authentication as an alternative to certificate authentication. Instead of authenticating each time with a Kerberos ticket, we can authenticate just once and retrieve a WireGuard configuration file. If the WireGuard configuration file expires, we try to renew it 6 weeks before the expiry date with a new Kerberos ticket.  
![image](https://user-images.githubusercontent.com/47246332/167580444-dbb7ec09-1541-4351-a5fe-17693e729813.png)
